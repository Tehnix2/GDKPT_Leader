...
local function OnTradeOpenedInPotSplitPhase()
    ...
    -- Reset previous trade data at the start of a new trade
    GDKPT.RaidLeader.PotSplitTrading.CurrentPotSplitTrade = {
        partner = nil,
        partnerBalance = 0,
        leaderGold = 0,
...
-------------------------------------------------------------------
-- This function is only called when a trade gets canceled in pot 
-- split phase
-------------------------------------------------------------------


local function OnTradeCancelledInPotSplitPhase()
    -- A canceled trade should be wiped
    GDKPT.RaidLeader.PotSplitTrading.CurrentPotSplitTrade = {
        partner = nil,
        partnerBalance = 0,
        leaderGold = 0,
...
potSplitTradeFrame:SetScript("OnEvent", function(self, event, ...)
    ...
    elseif event == "UI_INFO_MESSAGE" then
        ...
        elseif msg == ERR_TRADE_CANCELLED then 
            OnTradeCancelledInPotSplitPhase()
        end
    end
end)
