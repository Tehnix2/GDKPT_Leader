...
local function FindItemInBags(wonItem)
    ...
    -- For bulk items, use the stored bag/slot info directly
    if wonItem.isBulkItem and wonItem.bagID and wonItem.slotID then
        -- Verify item is still there
        local currentLink = GetContainerItemLink(wonItem.bagID, wonItem.slotID)
        if currentLink then
            local currentItemID = tonumber(currentLink:match("item:(%d+)"))
            if currentItemID == wonItem.itemID then
                return wonItem.bagID, wonItem.slotID
            end
        end
        -- If not found in expected location, fall through to hash search
    end
