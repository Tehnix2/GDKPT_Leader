...
local function SendActiveAuctions(activeAuctions, channel, targetPlayer, messageDelay, delayIncrement)
    ...
    for _, data in ipairs(activeAuctions) do
        ...
        if auction.isBulkAuction and auction.bulkItems then
            -- Send as BULK_AUCTION_START
            local itemListStr = ""
            for i, item in ipairs(auction.bulkItems) do
...
local function SendCompletedAuctions(completedAuctions, channel, targetPlayer, messageDelay, delayIncrement)
    ...
    for _, data in ipairs(completedAuctions) do
        ...
        C_Timer.After(messageDelay, function()
            -- Check if bulk auction
            if auction.isBulkAuction and auction.bulkItems then
                -- Send bulk auction start with 0 duration
                local itemListStr = ""
                for i, item in ipairs(auction.bulkItems) do
                    if i > 1 then itemListStr = itemListStr .. "," end
                    itemListStr = itemListStr .. string.format("%d:%d", item.itemID, item.stackCount)
            ...
            -- Send end message (same for both)
            C_Timer.After(0.25, function()
