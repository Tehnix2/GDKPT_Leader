...
-- Default to false - pot split only starts when all members paid
GDKPT.RaidLeader.PotSplit.StartPotSplitPhase = true --false
...
function GDKPT.RaidLeader.PotSplit.DistributePot()
    ...
    local playerShare = math.floor(totalPot / numSplits)   -- Problem: this rounds down to the closes gold value, silver and copper get scammed
    ...
    -- Apply the Cut to all raidmember's balance
    local distributedTo = 0
    for i = 1, numRaid do
        local name = GetRaidRosterInfo(i)
        if name then
    ...
    -- Add the share to all player balances and update the display in LeaderFrame
    if GDKPT.RaidLeader.UI.UpdateRosterDisplay then
        GDKPT.RaidLeader.UI.UpdateRosterDisplay()
    end
    ...
    if IsInRaid() then
        SendChatMessage(
            string.format(
                "[GDKPT] Pot Split initialized! Total Pot: %dg - Split by: %d - Each receives: %dg - Trade the RaidLeader for your gold!",
                totalPot, numSplits, playerShare),"RAID")
    end
end
...
SplitPotNowButton:SetScript("OnClick", function()
    ...
    StaticPopupDialogs["GDKPT_CONFIRM_FINALIZE"] = {
        text = "Do you want to split the pot now? \nThis will add the cut to all players.",
        button1 = "Yes, Finalize",
        button2 = "Cancel",
        OnAccept = function()
            if IsInRaid() then
                GDKPT.RaidLeader.PotSplit.DistributePot()
...
function GDKPT.RaidLeader.PotSplit.ShowAutoFillButtonOnValidTrade(partner, amountGold)
    ...
    LeaderAutoFillButton:SetText(string.format("AutoFill %dg", amountGold))
    LeaderAutoFillButton:Show()
end
